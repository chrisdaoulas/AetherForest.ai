...
# Authenticate to the Google Drive of the Service Account
    ...
    landsat2015 = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2').filterDate(four_months_before(time_start),time_start).filterBounds(globals()[project_geom]).filter(ee.Filter.lt('CLOUD_COVER',10)).aside(print).map(ndviLS).map(applyScaleFactors).median().clip(globals()[project_geom])
    ...
    sentinel2023 = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED') \
                .filterDate(eight_months_before(time_end), time_end) \
...
#kml2shape(kml) #fix to file
project = 'Kayapo'
project = 'Yanomami'
time_start = '2015-01-16'
time_end = '2024-04-16'
